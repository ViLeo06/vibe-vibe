...
**方案二：幂等键（Idempotency Key）。** 前端每次提交时生成一个唯一的请求 ID（比如 UUID），放在请求头里。后端收到请求后，不是简单地"先查一下有没有处理过"，而是要用**原子方式**记录这个 ID 的状态，避免并发请求同时穿过去。

一套更稳妥的通用做法是：

- **不存在**：原子性地写入一条"处理中"记录，开始执行创建逻辑
- **处理中**：说明同一个请求已经在路上了，直接返回"请求处理中，请稍后重试"，或者按业务约定等待结果
- **已完成**：直接返回第一次处理的结果，不再重复创建

这类记录通常还要设置**过期时间**，不然请求量一大，存储会一直涨。并且只有**成功完成**后，才把最终响应缓存下来；如果中途失败，一般应该允许客户端用同一个幂等键重试。

这是最通用的方案，适合没有天然唯一约束的创建接口。支付接口几乎都用这种方式，因为你绝对不想因为网络重试导致用户被扣两次钱。
...
- **幂等性防重复**：数据库唯一约束是最简单的方案；幂等键更通用，但生产里要配合原子写入、处理中状态和过期策略
